pipeline {
  ...
  stages{
    ...
    stage('Unit Tests') {
        steps{
            echo "------------>Unit Tests<------------"
            sh 'xcodebuild test -scheme CeibaAdn-Parking -configuration "Debug"  -destination "platform=iOS Simulator,name=iPhone 11,OS=14.5" -enableCodeCoverage YES | xcpretty -r junit --output build/reports/junit.xml'
        }
    }
